...
export class SalesController {
  ...
  @ApiResponse({
    ...
    description: 'Returns paginated sales data',
    type: Sale,
    isArray: true,
  })
  ...
  @Get('search')
  @ApiOperation({
    summary: 'Search sales with filters, pagination, and sorting',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Returns filtered and paginated sales data',
    type: Sale,
    isArray: true,
  })
  @ApiQuery({ name: 'page', required: false, type: Number })
  ...
  async search(@Query() searchParams: SaleSearchParams) {
  ...
  async update(
    ...
    @Body(new ValidationPipe({ transform: true })) updateSaleDto: UpdateSaleDto,
  ) {
